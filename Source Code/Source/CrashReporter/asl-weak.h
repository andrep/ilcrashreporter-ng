 ...
 */
aslclient asl_open(const char *ident, const char *facility, uint32_t opts) WEAK_IMPORT_ATTRIBUTE;

/*
 * Shuts down the current connection to the server.
 */
void asl_close(aslclient asl) WEAK_IMPORT_ATTRIBUTE;
 ...
 */
int asl_add_log_file(aslclient asl, int fd) WEAK_IMPORT_ATTRIBUTE;

/*
 * asl_remove_file: stop writing log messages to the given file descriptor
 * The file descripter is not closed by this routine.
 */
int asl_remove_log_file(aslclient asl, int fd) WEAK_IMPORT_ATTRIBUTE;
 ...
 */
int asl_set_filter(aslclient asl, int f) WEAK_IMPORT_ATTRIBUTE;
 ...
 */
const char *asl_key(aslmsg msg, uint32_t n) WEAK_IMPORT_ATTRIBUTE;

/*
 * asl_new: create a new log message.
 */
aslmsg asl_new(uint32_t type) WEAK_IMPORT_ATTRIBUTE;
 ...
 */
int asl_set(aslmsg msg, const char *key, const char *value) WEAK_IMPORT_ATTRIBUTE;
 ...
 * returns 0 for success, non-zero for failure
 */
int asl_unset(aslmsg msg, const char *key) WEAK_IMPORT_ATTRIBUTE;
 ...
 */
const char *asl_get(aslmsg msg, const char *key) WEAK_IMPORT_ATTRIBUTE;
...
int asl_log(aslclient asl, aslmsg msg, int level, const char *format, ...) __DARWIN_LDBL_COMPAT2(asl_log) __printflike(4, 5) WEAK_IMPORT_ATTRIBUTE;
#else
int asl_log(aslclient asl, aslmsg msg, int level, const char *format, ...) __printflike(4, 5) WEAK_IMPORT_ATTRIBUTE;
#endif
 ...
 */
#ifdef __DARWIN_LDBL_COMPAT2
int asl_vlog(aslclient asl, aslmsg msg, int level, const char *format, va_list ap) __DARWIN_LDBL_COMPAT2(asl_vlog) __printflike(4, 0) WEAK_IMPORT_ATTRIBUTE;
#else
int asl_vlog(aslclient asl, aslmsg msg, int level, const char *format, va_list ap) __printflike(4, 0) WEAK_IMPORT_ATTRIBUTE;
#endif
 ...
 * returns 0 for success, non-zero for failure
 */
int asl_send(aslclient asl, aslmsg msg) WEAK_IMPORT_ATTRIBUTE;

/*
 * asl_free: free a message 
 * msg:  an aslmsg to free
 */
void asl_free(aslmsg msg) WEAK_IMPORT_ATTRIBUTE;
 ...
 * returns 0 for success, non-zero for failure
 */
int asl_set_query(aslmsg msg, const char *key, const char *value, uint32_t op) WEAK_IMPORT_ATTRIBUTE;
 ...
 */
aslresponse asl_search(aslclient asl, aslmsg msg) WEAK_IMPORT_ATTRIBUTE;
 ...
 */
aslmsg aslresponse_next(aslresponse r) WEAK_IMPORT_ATTRIBUTE;

/*
 * aslresponse_free: Free a response returned from asl_search() 
 * a: a response returned from asl_search()
 */
void aslresponse_free(aslresponse a) WEAK_IMPORT_ATTRIBUTE;
